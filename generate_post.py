...
At the end of your response, you must include 3 to 5 SEO-friendly tags as a valid Python list.
...
"""
    ...
    try:
        if "[CONTENT]" in full_output and "[TAGS]" in full_output:
            content = full_output.split("[TAGS]")[0].replace("[CONTENT]", "").strip()
            tags_str = full_output.split("[TAGS]")[1].strip()
            tags = ast.literal_eval(tags_str)
            if not isinstance(tags, list):
                raise ValueError("Tags is not a list.")
            return content, tags
        else:
            raise ValueError("Missing expected markers")
    except Exception as e:
        print("[!] Failed to parse GPT output:", e)
        return full_output, ["ai", "blog", "guide"]

def generate_image(topic):
    try:
        res = client.images.generate(
            prompt=f"A clean and abstract blog thumbnail image concept related to '{topic}', white background, no people, no logos, no text",
            n=1,
            size="512x512"
        )
        ...
        # Fallback image
        return "https://dummyimage.com/512x512/eeeeee/000000&text=Blog+Thumbnail"
